# Combine all slinky_scale_live functions into 1# this is the long slinky live function
...
slinky_scale_live3 <- function(list_esets, cell_prop=NULL,
  ...
  server_fun <- function(input, output) {
    ...
    output$d2_int_plot <- renderPlotly({
      p <- plot_2d_int_button() + event_register(p, 'plotly_hover')
      p$elementId <- NULL
      p
    })
...
slinky_scale_live3(list_sample[1:3])
